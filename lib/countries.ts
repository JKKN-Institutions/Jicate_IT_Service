/**
 * Master country list for every contact / demo form across the site.
 *
 * `countryData` carries each country's ISO-3166 alpha-2 code and international
 * dial code so the form can auto-fill a phone prefix + flag when a country is
 * chosen. `countries` (names only) is kept for backward compatibility.
 *
 *   import { countryData, countries, flagEmoji, dialForCountry } from "@/lib/countries";
 */
export interface Country {
  /** Display name (also the <option> value). */
  name: string;
  /** ISO-3166 alpha-2 code (used to derive the flag emoji). */
  iso2: string;
  /** International dial code, incl. leading "+". */
  dial: string;
}
...
/** Country names only — backward-compatible with earlier imports. */
export const countries: string[] = countryData.map((c) => c.name);
...
/** Look up a country's dial code by display name. */
export function dialForCountry(name: string): string {
  return countryData.find((c) => c.name === name)?.dial ?? "";
}
