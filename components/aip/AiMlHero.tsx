...
import { aimlHero } from "@/content/aip";
    ...
    <section className="relative isolate flex min-h-[100svh] items-end overflow-hidden bg-near-black text-offwhite">
      ...
      <Container className="flex w-full items-end justify-between gap-l pb-[40px] pt-2xl tablet:pb-[40px]">
        ...
        <div className="flex items-center gap-[0.06em] text-offwhite">
          <PlaceholderMark className="h-[clamp(4rem,31vw,28rem)] w-auto shrink-0" />
          <span className="font-display text-[clamp(4rem,31vw,28rem)] font-normal leading-[0.8] tracking-[-0.05em]">
            {aimlHero.wordmark}
          </span>
        </div>

        {/* Mono micro-label column. */}
        <ul className="hidden shrink-0 flex-col gap-l text-left tablet:flex">
          {aimlHero.metaGroups.map((group, i) => (
            <li
              key={i}
              className="border-l border-offwhite/30 pl-[14px] font-mono text-[0.6875rem] font-medium uppercase leading-[1.35] tracking-[0.04em] text-offwhite/85"
            >
              {group.map((line, j) => (
