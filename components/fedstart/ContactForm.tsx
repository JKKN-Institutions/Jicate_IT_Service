...
/**
 * Contact Us — a light lead-capture form. Giant heading on the left, single-column
 * full-width fields on the right (with FedStart-specific qualifying questions).
 * Asterisk before label. Static UI.
 */
      ...
      <Container>
        <div className="grid-12 gap-y-2xl">
          <Reveal className="col-span-12 desktop:col-span-5">
            <GiantHeading size="anchor">{contact.heading}</GiantHeading>
          </Reveal>
          ...
          <Reveal index={1} className="col-span-12 desktop:col-span-6 desktop:col-start-7">
            <form className="flex flex-col gap-l" noValidate>
              ...
              <label className="flex flex-col gap-xs">
                <span className={labelCls}>{contact.contextLabel}</span>
                <textarea name="context" rows={3} className={fieldCls} />
              </label>

              {contact.questions.map((q) => (
                <label key={q.name} className="flex flex-col gap-xs">
                  <span className={labelCls}>{q.label}</span>
                  <select name={q.name} className={fieldCls} defaultValue="">
                    <option value="" disabled>
                      Select…
                    </option>
                    {q.options.map((o) => (
              ...
              <label className="flex flex-col gap-xs">
                <span className={labelCls}>{contact.deployLabel}</span>
                <textarea name="deployment" rows={3} className={fieldCls} />
              </label>
