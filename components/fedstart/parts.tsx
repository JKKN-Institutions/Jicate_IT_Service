...
/** A leading "↳" arrow item — used in the bullet lists across the page. */
export function ArrowItem({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("flex items-start gap-s text-body leading-[1.45] text-ink", className)}>
 ...
 * `play` shows a ▶ control to suggest a video; otherwise a small image glyph.
 */
export function MediaPlaceholder({
  ...
  return (
    <div
      ...
      aria-label={label}
      className={cn(
        "flex w-full items-center justify-center overflow-hidden rounded-[2px] bg-ink/[0.06]",
        className,
      )}
    >
      {play ? (
        <span className="flex items-center gap-s text-body-lg text-ink">
          <span aria-hidden className="flex size-[44px] items-center justify-center rounded-full border border-ink/40">
            ▶
          </span>
        </span>
      ) : (
