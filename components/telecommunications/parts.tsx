  ...
  return (
    <Tag
      className={cn(
        ...
        size === "band" && "text-[clamp(1.875rem,1rem+2.6vw,2.778rem)] leading-[1.05]",
        className,
      )}
    >
      {children}
...
export function InkLink({
  ...
  return (
    <a
      href={href}
      className={cn(
        "group inline-flex items-center justify-between gap-s border-b border-ink/25 pb-s text-body text-ink transition-colors duration-200 hover:border-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent",
        className,
      )}
    >
      <span>{children}</span>
